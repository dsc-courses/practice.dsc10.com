...
You are working in a spreadsheet editor and you highlight several consecutive rows of your spreadsheet. The first highlighted row is row 543 and the last highlighted row is row 897. How many rows are highlighted?
...
In general, imagine highlighting all $m$ rows up to and including row $m$. That would be $m$ highlighted rows. But if we only start highlighting at row $n$, this includes $n-1$ rows that should not be counted. So there are $m-(n-1) = m - n + 1$ rows highlighted when we highlight rows $n$ through $m$.
