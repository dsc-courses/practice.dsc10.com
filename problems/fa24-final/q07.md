...
We want to group the `sales` DataFrame by the `day_of_week` column that was created in blank (a) to perform some aggregate on each day of the week.
...
We first take one column (doesn't have to be `"ISBN"`, we just chose it arbitrarily, because all columns have the same counts) that contains the count of rows for each day of the week and is indexed by the days in alphabetical order after the groupby. We then divide this column by the total number of rows to convert the counts to proportionsa. We want to compute the statistic that we've chosen above, **the mean of the absolute differences in proportions between the distribution of books sold by day and Hargen's proposed distribution**. Since the order of the days already match (both in alphabetical order), we can simply subtract one from the other to get the difference in proportions. We then turn the differences into absolute differences with `np.abs` and get the mean using `.mean()`.
...
For hypothesis testing, we simulate based on the null distribution, which is the distribution that Hargen proposed. For each of the simulated distribution of proportions , we calculate the statistic we chose above, which is **the mean of the absolute differences in proportions between the simulated distribution and Hargen's proposed distribution**. After thousands of simulations, we have calculated thousands of mean of absolute differences in proportions, each between one simulated distribution and Hargen's proposed distribution. Lastly, we compare the statistic for our observed distribution with the thousands of statistics from the simulated distributions to see whether **the observed distribution of books sold by day looks like a typical simulated distribution.**
