...
We want to count how many sales occurred on each day of the week, or, how many rows are in `sales` that belong to each day, so we use `count()` after grouping by `day_of_week`.
