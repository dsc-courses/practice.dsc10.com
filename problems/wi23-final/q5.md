...
**Option 3:** It is impossible for `resamp`’s minimum to be less than `samp`’s minimum. This is because all of `resamp`’s values come from `samp`. That means there cannot be a smaller value inside of `resamp` that never appears in `samp`.
