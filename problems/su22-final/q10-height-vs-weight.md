...
Snippet 4: Note that this code is just the same as Snippet 1, except we're now directly computing the product of each corresponding pai individually. Hence this Snippet works.
...
Sam computest the following statistics for his sample:
...
The best fit line in original units are given by `y = mx + b` where `m = r * (SD of y) / (SD of x)` and `b = (mean of y) - m * (mean of x)`. Let `c` be the SD of y, which we're trying to find, then our best fir line is now `y = (0.8*c/8)x + (50-(0.8*c/8)*15)`. Plugging the two values they gave us into our best fit line gives `45 = 0.1*c*10 + (50 - 1.5*c)` which simplifies to `45 = 50 - 0.5*c` which gives us an answer of `c = 10`.
# END SOLUTION
...
Option 1: We cannot determine whether two variables are linear simply from a line of best fit (which just so happens to find the best linear relationship between two varaibles)

Option 2: To calculate the root mean squared error, we need the acutaly data points so we can calculate residual values. Seeing that we don't have access to the data points, we cannot say that the root mean squared error of the best-fit line is smaller than 5.
