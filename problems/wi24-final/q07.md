...
<!--To test this pair of hypotheses, we need a test statistic that is large when the data suggests that the null hypothesis isn't true, and small when the null hypothesis is suggested to be true. Now let's look at each option:
1. 
2. This statistic is large when American gymnasts win more Olympic medals than Romanian ones (1/1). It get's smaller as American gymnasts win an equal amount of medals as Romanian ones (1/2), but then it gets even smaller as American gymnasts win less medals than Romanian ones (1/100, for example). This means that when Americans win way less medals than Romanians, this test statistic would still suggest that we fail to reject the hypothesis that Americans win the same amount of medals as Romanians. 
3. This test statistic is small when the null hypothesis is true, and large when the null hypothesis isn't true. Therefore, this would be an effective test statistic to test the hypotheses.
4. 
-->
